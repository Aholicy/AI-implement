import json
from envs.bq_src.operator_env.car_A_Env import Car_a_Env
from envs.bq_src.operator_env.car_B_Env import Car_b_Env
from envs.bq_src.operator_env.people_Env import People_Env
from envs.bq_src.map.map import Map
...
class War_Env(MultiAgentEnv):
    def __init__(self, game_agent_id, game_map_id, owner, state_last_action=False):
        ...
        self.use_invalid = True #False
        if self.use_invalid:
            # move 0-5 shoot 6-8 stop 9 invalid 10
            self.n_actions = 11
        else:
            self.n_actions = 10
        print('env is ready')
            ...
            if self.agents_alive_list[0][agent_id] != None:
                ...
                for enemy_id in self.agents_alive_list[1].keys():
                    if self.agents_alive_list[1][enemy_id] != None:
                        ...
                        if distance <= self.agents_alive_list[0][agent_id].can_be_seen:
                        ...
                        if distance <= self.agents_alive_list[1][enemy_id].can_be_seen:
    ...
    def get_obs_agent(self, agent_id):
        # 一共6个智能体，每个智能体14个特征信息
        # 观察值与状态值都是开这么多空间，但是观察值中敌方3个智能体的部分信息为0
    ...
    def step(self, actions):
        ...
        actions_ene = self.action_space_sample(1-self.owner)
        ...
        if done:
            ...
            self.battles_game += 1
            print('my_color:', self.owner, '  end_my_blood:', aly_blood_1, '  end_ene_blood:', ene_blood_1)
            if win_mark_list[self.owner] == False and win_mark_list[1 - self.owner] == True:
                self.win_n += 1
    ...
    def get_final_score(self):
        aly_blood, ene_blood = self.get_blood_score()
        score = {
            "aly_blood": aly_blood,
            "ene_blood": ene_blood
        }
