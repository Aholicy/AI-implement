...
class War_Env_hard1(MultiAgentEnv):
    ...
    def action_space_sample(self, knAI_color):
        ...
        for ene_id in self.agents_alive_list[knAI_color]:
            # print(self.agents_alive_list[knAI_color])
            # if len(self.agents_alive_list[knAI_color]) < 3:
            #     print()
    ...
    def step(self, actions):
        ...
        actions_ene = self.action_space_sample_AI1(1-self.owner)
        ...
        for agent_id, action in enumerate(actions_int):
            ...
            agent_id = self.index_agentid_dict[self.owner][agent_id]  # 转化id
            avail_actions = self.get_avail_agent_actions_fact(agent_id)
