...
class ParallelRunner:
    ...
    def get_final_score(self):
        for parent_conn in self.parent_conns:
            parent_conn.send(("get_final_score", None))

        aly_blood, ene_blood = [], []
        for parent_conn in self.parent_conns:
            env_final_score = parent_conn.recv()
            aly_blood.append(env_final_score["aly_blood"])
            ene_blood.append(env_final_score["ene_blood"])

        return sum(aly_blood) / len(aly_blood), sum(ene_blood) / len(ene_blood)
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        elif cmd == "get_final_score":
            remote.send(env.get_final_score())
        else:
