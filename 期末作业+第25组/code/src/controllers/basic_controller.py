...
class BasicMAC:
    def __init__(self, scheme, groups, args):
        ...
        self.agent_output_type = args.agent_output_type  # 输出Q值还是概率
    ...
    def _build_inputs(self, batch, t):
        ...
        inputs = th.cat([x.reshape(bs, self.n_agents, -1) for x in inputs], dim=-1) # [8,3,99]
        return inputs

    def _get_input_shape(self, scheme):
        input_shape = scheme["obs"]["vshape"] # 85
        if self.args.obs_last_action:
            input_shape += scheme["actions_onehot"]["vshape"][0] # 11
        if self.args.obs_agent_id:
