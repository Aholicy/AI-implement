...
class EpisodeBatch:
    def __init__(self,
                 ...
                 groups,           # 智能体数量
                 batch_size,       # 并行环境数量
                 max_seq_length,   # 环境steps上限
                 data=None,
                 preprocess=None,  # 动作Onehot形式维数（scheme中的动作维度为1）
                 device="cpu"):
    ...
    # 根据各数据维度信息为data开辟空间，data中包含batch size个数据，
    # 这里的batch size表示并行环境数量，每个环境中每个数据长度为max_seq_length
    def _setup_data(self, scheme, groups, batch_size, max_seq_length, preprocess):
        if preprocess is not None:
            # preprocess = {"actions": ("actions_onehot", [OneHot(out_dim=args.n_actions)])}
            # 在各数据维度信息中加入动作onehot向量的维度信息
            for k in preprocess:
                assert k in scheme
    ...
    # 数据，没有终止的环境（并行时），当前环境步数（并行时为最大的），数据不全（并行时）
...
class ReplayBuffer(EpisodeBatch):
    ...
    # EpisodeBatch大小为batch_size*max_seq_length，batch_size为并行环境数量
    # ReplayBuffer大小为buffer_size*max_seq_length，buffer_size为缓存池存储轨迹数量上限
    # 在episode环境中每次只存入一条轨迹
    # 在parallel环境中每次存入并行环境数量的轨迹，同时需要用bs来标记当前更新的是哪些环境的数据
    # 在ReplayBuffer中bs则用来标记当前更新的多条轨迹的下标
