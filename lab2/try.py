...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv3 = nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1)  # 2层， 输入通道in_channels 要等于上一层的 out_channels
        self.pool2 = nn.MaxPool2d(2, 2)  # Max pooling over a (2, 2) window 即最大池化层
        # 接着三个全连接层
        self.fc1 = nn.Linear(128 * 7 * 7, 512)  # 全连接层的输入特征维度为64*7*7，因为上一层Conv2d的out_channels=64，两个池化，所以是7*7而不是14*14
